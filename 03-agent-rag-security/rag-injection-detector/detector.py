...
INJECTION_PATTERNS = {
    "覆盖指令": r"忽略.*(之前|以上|所有).*(规则|指令|设定)|ignore\s+(all\s+)?(previous|prior)\s+instructions?",
    "伪系统标记": r"\[系统\]|\[system\]|<\|system\|>|###\s*system",
    "索取机密": r"(密钥|秘密|key|token|密码|password|管理员)",
    "越权承诺": r"(开通全部权限|授予权限|grant.*access|你现在拥有)",
    "改变行为": r"(对(任何人|所有人)都回答|从现在起.*回答|always respond)",
}
